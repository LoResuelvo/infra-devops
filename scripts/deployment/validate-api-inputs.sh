...
image_pattern='^ghcr\.io/loresuelvo/api@sha256:[a-f0-9]{64}$'

[[ "$caller_repository" == "LoResuelvo/loresuelvo-api" ]] || {
...
[[ "$image_ref" =~ $image_pattern ]] || {
  echo "Image reference must be an immutable LoResuelvo API digest." >&2
  exit 1
}
